...
def load_dyn_sys_class(dataset_name):
    ...
    dataset_classes = {
        ...
        'GissingerReversals': GissingerReversals,
...
# Noorain's additional dataset
class GissingerReversals(DynSys):
    def __init__(self, state_dim=3, mu=0.119, nu=0.1, gamma=0.1, params=None):
        """
    ...
    def rhs(self, t, x):
        ...
        return torch.cat([dQ, dD, dV], dim=1)
    ...
    def get_inits(self, size):
        ...
        """
        Q0 = torch.empty(size, 1).uniform_(-1.0, 1.0)
        D0 = torch.empty(size, 1).uniform_(-1.0, 1.0)
        V0 = torch.empty(size, 1).uniform_(self.gamma - 1.0, self.gamma + 1.0)
        return torch.cat([Q0, D0, V0], dim=1)
